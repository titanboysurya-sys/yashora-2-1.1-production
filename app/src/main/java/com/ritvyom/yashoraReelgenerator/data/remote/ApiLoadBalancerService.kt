...
object ApiLoadBalancerService {
    ...
    private val premiumPexelsKeys = listOf(
        ...
        "NyEXcmIT8bs3CN4dLNForDzJQVyVrWCMRfDLjfgtKYX9t3z8dUm1QHNl"  // Premium Backup 2
    )

    private val premiumPixabayKeys = listOf(
        "56218545-6d93003a94318db8a7f29dba1", // Premium Primary
        "56218545-6d93003a94318db8a7f29dba1", // Premium Backup 1
        "56218545-6d93003a94318db8a7f29dba1"  // Premium Backup 2
    )

    private val premiumUnsplashKeys = listOf(
        "J6C-j-OjDXft4dMbIm96LlAX4ZqUuViErwbOENvkt4Q", // Premium Primary
        "J6C-j-OjDXft4dMbIm96LlAX4ZqUuViErwbOENvkt4Q", // Premium Backup 1
        "J6C-j-OjDXft4dMbIm96LlAX4ZqUuViErwbOENvkt4Q"  // Premium Backup 2
    )
    ...
    /**
     * Resolves the best key for Pexels.
     * Tier 1: User-provided key (always prioritized, never rotated away).
     * Tier 2: Low-traffic standard app key.
     * Tier 3: Load-balanced premium pool.
     */
    fun resolvePexelsApiKey(envKey: String, userCustomKey: String = ""): String {
        val candidate = (if (userCustomKey.isNotBlank()) userCustomKey else envKey).trim().removeSurrounding("\"").removeSurrounding("'")
        if (candidate.isNotEmpty() && candidate.length >= 16 && !candidate.startsWith("YOUR_") && candidate !in premiumPexelsKeys) {
            Log.i(TAG, "Pexels: Prioritizing User-provided custom API key.")
            return candidate
        }
    ...
    /**
     * Resolves the best key for Pixabay.
     * Tier 1: User-provided key (always prioritized, never rotated away).
     * Tier 2: Low-traffic standard app key.
     * Tier 3: Load-balanced premium pool.
     */
    fun resolvePixabayApiKey(envKey: String, userCustomKey: String = ""): String {
        val candidate = (if (userCustomKey.isNotBlank()) userCustomKey else envKey).trim().removeSurrounding("\"").removeSurrounding("'")
        if (candidate.isNotEmpty() && candidate.length >= 16 && !candidate.startsWith("YOUR_") && candidate !in premiumPixabayKeys) {
            Log.i(TAG, "Pixabay: Prioritizing User-provided custom API key.")
            return candidate
        }
    ...
    /**
     * Resolves the best key for Unsplash.
     * Tier 1: User-provided key (always prioritized, never rotated away).
     * Tier 2: Low-traffic standard app key.
     * Tier 3: Load-balanced premium pool.
     */
    fun resolveUnsplashApiKey(envKey: String, userCustomKey: String = ""): String {
        val candidate = (if (userCustomKey.isNotBlank()) userCustomKey else envKey).trim().removeSurrounding("\"").removeSurrounding("'")
        if (candidate.isNotEmpty() && candidate.length >= 16 && !candidate.startsWith("YOUR_") && candidate !in premiumUnsplashKeys) {
            Log.i(TAG, "Unsplash: Prioritizing User-provided custom API key.")
            return candidate
        }
